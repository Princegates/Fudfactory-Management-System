          ...
          <tbody>
            {products.map((p) => (
              <tr key={p.id} className="border-b border-brand-50">
                ...
                <td className="p-3">
                  <ProductRowControls productId={p.id} price={p.price} isAvailable={p.isAvailable} isFeatured={p.isFeatured} />
                </td>
              </tr>
            ))}
            {products.length === 0 && (
              <tr><td colSpan={3} className="p-6 text-center text-cocoa-900/50">No products yet — add your first one above.</td></tr>
            )}
          </tbody>
        </table>
      </div>
